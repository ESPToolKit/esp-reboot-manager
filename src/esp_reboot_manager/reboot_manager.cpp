...
void ESPRebootManager::deinit() {
    ...
    while(
        taskHandle_.load(std::memory_order_acquire) != nullptr &&
...
RebootSubmitResult ESPRebootManager::requestReboot(const char* reason, uint32_t delayMs) {
    ...
    {
        std::lock_guard<std::mutex> lock(mutex_);
        if( status_.load(std::memory_order_acquire) != RebootRequestStatus::Idle || pendingRequest_.pending ){
...
void ESPRebootManager::compactGuards(std::vector<GuardEntry>& entries) {
    entries.erase(
        std::remove_if(entries.begin(), entries.end(), [](const GuardEntry& entry) { return !entry.active; }),
        entries.end()
    );
}
...
void ESPRebootManager::compactEvaluations(std::vector<EvaluationEntry>& entries) {
    entries.erase(
        std::remove_if(entries.begin(), entries.end(), [](const EvaluationEntry& entry) { return !entry.active; }),
        entries.end()
    );
}
