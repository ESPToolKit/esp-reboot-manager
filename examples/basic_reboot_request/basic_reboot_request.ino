...
void setup() {
    ...
    rebootManager.onEvaluation([](const RebootEvaluation& evaluation) {
        if (evaluation.accepted) {
            Serial.printf("[reboot] accepted id=%lu reason=%s delay=%lu\n",
        ...
        Serial.printf("[reboot] rejected id=%lu code=%u blocker=%s detail=%s\n",
                      static_cast<unsigned long>(evaluation.requestId),
    ...
    Serial.printf("submit status=%u requestId=%lu\n",
